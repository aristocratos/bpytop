...
def strtobool(val: str) -> bool:
	...
	try:
		...
		raise ValueError(f"invalid type {type(val)} for truth value {val}") from e
...
class Config:
	...
	def __init__(self, path: str):
		...
		if "version" not in conf:
			self.recreate = True
			self.info.append(
				'Config file malformatted or missing, will be recreated on exit!'
			)
		elif conf["version"] != VERSION:
			self.recreate = True
			self.info.append(
				'Config file version and bpytop version mismatch, will be recreated on exit!'
			)
		for key in self.keys:
			if key in conf and conf[key] != "_error_":
	...
	def load_config(self) -> Dict[str, Union[str, int, bool]]:
		...
		if (
			...
			and new_config["proc_sorting"] not in self.sorting_options
		):
			new_config["proc_sorting"] = "_error_"
			self.warnings.append(f'Config key "proc_sorted" didn\'t get an acceptable value!')
		if (
			"log_level" in new_config
			and new_config["log_level"] not in self.log_levels
		):
		...
		if (
			...
			and new_config["cpu_sensor"] not in self.cpu_sensors
		):
			new_config["cpu_sensor"] = "_error_"
			self.warnings.append(
				'Config key "cpu_sensor" does not contain an available sensor!'
			)
		if "shown_boxes" in new_config and new_config["shown_boxes"] != "":
			for box in new_config["shown_boxes"].split():  # type: ignore
				if box not in ["cpu", "mem", "net", "proc"]:
					new_config["shown_boxes"] = "_error_"
					self.warnings.append('Config key "shown_boxes" contains invalid box names!')
					break
		for cpu_graph in ["cpu_graph_upper", "cpu_graph_lower"]:
			if (
				cpu_graph in new_config
				and new_config[cpu_graph] not in self.cpu_percent_fields
			):
		...
		if (
			...
			and new_config["temp_scale"] not in self.temp_scales
		):
			new_config["temp_scale"] = "_error_"
			self.warnings.append(
				'Config key "temp_scale" does not contain a recognized temperature scale!'
			)
		return new_config
			...
			for box_class in Box.__subclasses__():
				for box_name in Box.boxes:
					if box_name in str(box_class).capitalize():
						if (
							(box_name != "cpu" or "proc" not in Box.boxes)
							and (box_name != "net" or "mem" not in Box.boxes)
							and w_p + box_class.width_p <= 100
						):
							w_p += box_class.width_p
							cls.min_width += getattr(box_class, "min_w", 0)
						if (
							box_name not in ["mem", "net"] or "proc" not in Box.boxes
...
class Key:
	...
	@classmethod
	def last(cls) -> str:
		return cls.list.pop() if cls.list else ""

	@classmethod
	def get(cls) -> str:
		return cls.list.pop(0) if cls.list else ""
...
def buffer(cls, name: str, *args: str, append: bool = False, now: bool = False, z: int = 100,
	...
	cls.once[name] = once
	if name not in cls.z_order or z != 100: cls.z_order[name] = z
	string = "".join(args) if args else ""
	if only_save:
...
class Color:
	...
	* Values:  .hexa: str  |  .dec: Tuple[int, int, int]  |  .red: int  |  .green: int  |  .blue: int  |  .depth: str  |  .escape: str
	...
	def truecolor_to_256(rgb: Tuple[int, int, int], depth: str = "fg") -> str:
		...
		return (
			f'{pre}{232 + greyscale[0]}m'
			if greyscale[0] == greyscale[1] == greyscale[2]
			else f'{pre}{round(rgb[0] / 51) * 36 + round(rgb[1] / 51) * 6 + round(rgb[2] / 51) + 16}m'
		)
		...
		if hexa:
			try:
				if len(hexa) == 3:
					...
					color = (
						f'\033[{dint};2;{c};{c};{c}m'
						if CONFIG.truecolor and not LOW_COLOR
						else f'{Color.truecolor_to_256(rgb=(c, c, c), depth=depth)}'
					)
				elif len(hexa) == 7:
					if CONFIG.truecolor and not LOW_COLOR:
				...
				errlog.exception(f'{e}')
		elif CONFIG.truecolor and not LOW_COLOR:
...
class Theme:
	...
	def _load_theme(self, theme: str):
		...
		if "graph_text" not in tdict and "inactive_fg" in tdict:
			tdict["graph_text"] = tdict["inactive_fg"]
		if "meter_bg" not in tdict and "inactive_fg" in tdict:
			tdict["meter_bg"] = tdict["inactive_fg"]
		if "process_start" not in tdict and "cpu_start" in tdict:
...
class Banner:
	...
	def draw(cls, line: int, col: int = 0, center: bool = False, now: bool = False):
		...
		out: str = "".join(f'{Mv.to(line + n, col)}{o}' for n, o in enumerate(cls.out))
		out += f'{Term.fg}'
		if now:
			Draw.out(out)
		else:
	...
	* __call__ : same as add
	...
	def __init__(self, width: int, height: int, color: Union[List[str], Color, None], data: List[int],
		...
		if max_value:
			...
			self.max_value = 0
		self.color_max_value = color_max_value or self.max_value
		self.colors: List[str] = []
		if isinstance(color, list) and height > 1:
			color_scale = (
				int(100.0 * self.max_value / self.color_max_value)
	...
	def _create(self, data: List[int], new: bool = False):
		...
		for h in range(self.height):
			...
			for v in range(len(data)):
				...
				for val, side in ([self.last, "left"], [data[v], "right"]):  # type: ignore
					...
					if (
						self.no_zero
						and (not new or v != 0 or side != "left")
						and h == self.height - 1
						and value[side] < 1
						and (not self.round_up_low or val != 0)
					): value[side] = 1
		...
		if self.height == 1:
			self.out += f'{(THEME.inactive_fg if self.last < 5 else self.colors[self.last]) if self.colors else ""}{self.graphs[self.current][0]}'
		elif self.height > 1:
			for h in range(self.height):
				if h > 0: self.out += f'{Mv.d(1)}{Mv.l(self.width)}'
				self.out += f'{self.colors[h] if self.colors else ""}{self.graphs[self.current][((self.height - 1) - h if self.invert else h)]}'
...
class Meter:
	...
	def __call__(self, value: Union[int, None]) -> str:
		...
		if value > 100 or value < 0:
			value = 100
		self.out = self.saved[value] if value in self.saved else self._create(value)
		return self.out
	...
	def _create(self, value: int) -> str:
		...
		for i in range(1, self.width + 1):
			if value >= round(i * 100 / self.width):
				out += f'{self.color_gradient[(round(100 - (i * 100 / self.width)) if self.invert else round(i * 100 / self.width))]}{Symbol.meter}'
			else:
...
class Box:
	...
	def draw_update_ms(cls, now: bool = True):
		if "cpu" not in cls.boxes: return
	...
	def draw_clock(cls, force: bool = False):
		if "cpu" not in cls.boxes or not cls.clock_on: return
		cls.c_counter += 1
		...
		if now and not Menu.active and (
			CONFIG.show_battery and hasattr(psutil, "sensors_battery") and psutil.sensors_battery()):
...
class CpuBox(Box, SubBox):
	...
	def _calc_size(cls):
		...
		cls.box_height = min(cls.box_height, cls.height - 2)
	...
	def _draw_bg(cls) -> str:
		...
		return (f'{create_box(box=cls, line_color=THEME.cpu_box)}'
				...
				f'{create_box(x=cls.box_x, y=cls.box_y, width=cls.box_width, height=cls.box_height, line_color=THEME.div_line, fill=False, title=CONFIG.custom_cpu_name[:cls.box_width - 14] if CONFIG.custom_cpu_name else CPU_NAME[:cls.box_width - 14])}')
	...
	def battery_activity(cls) -> bool:
		...
		status: str = "not_set"
		if cls.battery_path:
			status = readfile(f"{cls.battery_path}status", default="not_set")
			...
			if CONFIG.show_disks and mem.disks:
				if CONFIG.show_io_stat or CONFIG.io_mode:
					...
					for name in mem.disks.keys():
						...
						Graphs.disk_io[name] = {
							_name: Graph(width=cls.disks_width - (0 if CONFIG.io_mode else 6), height=_height,
		...
		try:
			...
			for name in cls.mem_names:
				...
				if cls.mem_size > 2:
					...
					cy += cls.graph_height + 1 if cls.graph_height else 2
				else:
	...
	def _draw_fg(cls):
		...
		if cls.resized or cls.redraw:
			...
			out_misc += (f'{Mv.to(y - 1, x + 8)}{THEME.proc_box(Symbol.h_line * (w - 9))}' +
						 (
							 f"{Mv.to(dy + 7, dgx + dgw + 1)}{THEME.proc_box(Symbol.div_down)}" if proc.detailed else "") +
			...
			if not "f" in Key.mouse or cls.resized: Key.mouse["f"] = [[x + 6 + i, y - 1] for i in range(
				2 + len(proc.search_filter[-10:]) if proc.search_filter else 6)]
	...
	def _collect(cls):
		...
		for disk in psutil.disk_partitions(all=CONFIG.use_fstab or not CONFIG.only_physical):
			...
			if CONFIG.use_fstab and disk.mountpoint not in cls.fstab_filter:
				continue
			disk_name = "root" if disk.mountpoint == "/" else disk.mountpoint.rsplit('/', 1)[-1]
...
def create_box(x: int = 0, y: int = 0, width: int = 0, height: int = 0, title: str = "", title2: str = "",
	...
	if title:
		numbered: str = f'{THEME.hi_fg(SUPERSCRIPT[num])}' if num else ""
